...
public class BaseService {
  ...
  protected TagslutApiSdkConfig config;
   ...
   */
  public BaseService(OkHttpClient httpClient, TagslutApiSdkConfig config) {
  ...
  /**
   * Resolves the access token from the configuration hierarchy.
   *
   * @param resolvedConfig The merged request configuration (may be null)
   * @return The resolved access token
   */
  protected String resolveAccessToken(RequestConfig resolvedConfig) {
    if (resolvedConfig != null && resolvedConfig.getAccessToken() != null) {
      return resolvedConfig.getAccessToken();
    }
    return this.config.getAccessToken();
  }

  /**
   * Resolves the basic auth config from the configuration hierarchy.
   *
   * @param resolvedConfig The merged request configuration (may be null)
   * @return The resolved basic auth config
   */
  protected BasicAuthConfig resolveBasicAuthConfig(RequestConfig resolvedConfig) {
    if (resolvedConfig != null && resolvedConfig.getBasicAuthConfig() != null) {
      return resolvedConfig.getBasicAuthConfig();
    }
    return this.config.getBasicAuthConfig();
  }
