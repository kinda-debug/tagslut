...
Usage:
    python -m dedupe.tools.track_hub_cli track-by-path /path/to/file.flac --db /path/to/music.db
    python -m dedupe.tools.track_hub_cli track-by-key "isrc:USRC12345678" --db /path/to/music.db
    python -m dedupe.tools.track_hub_cli list-files-for-key "isrc:USRC12345678" --db /path/to/music.db
    python -m dedupe.tools.track_hub_cli find-by-isrc USRC12345678 --db /path/to/music.db
    python -m dedupe.tools.track_hub_cli find-by-provider spotify 4iV5W9uYEdYUVa79Axb7Rh --db /path/to/music.db
...
Note: This module lives under dedupe/tools/ to avoid colliding with the main
Click-based CLI under dedupe/cli/.
"""
...
def cmd_propose_plan(args: argparse.Namespace) -> None:
    ...
    Generate a YAML (and optional CSV) dedupe plan based on duplicate analysis.
    """
    conn = _connect_db(args.db)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="dedupe.tools.track_hub_cli",
        description="Inspect the track hub (library_tracks, library_track_sources) directly.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    python -m dedupe.tools.track_hub_cli track-by-path /Music/Artist/Track.flac --db ./music.db
    python -m dedupe.tools.track_hub_cli track-by-key "isrc:USRC12345678" --db ./music.db
    python -m dedupe.tools.track_hub_cli track-by-key "spotify:4iV5W9uYEdYUVa79Axb7Rh" --db ./music.db
    python -m dedupe.tools.track_hub_cli list-files-for-key "isrc:USRC12345678" --db ./music.db
    python -m dedupe.tools.track_hub_cli find-by-isrc USRC12345678 --db ./music.db
    python -m dedupe.tools.track_hub_cli find-by-provider spotify 4iV5W9uYEdYUVa79Axb7Rh --db ./music.db
    python -m dedupe.tools.track_hub_cli diagnose-duplicates --db ./music.db --min-files 2 --limit 50
        """,
    )
    ...
    parser_plan = subparsers.add_parser(
        "propose-plan",
        help="Generate a YAML (and optional CSV) dedupe plan for duplicate files.",
