sing the collection run results, analyze the actual API responses and generate bulk tests for all requests in the collection.



List of collection runs retrieved successfully.



Retrieved 1 iteration(s). All passed.

Now I have the collection run data with actual response structures. Let me analyze the patterns and create comprehensive tests. I'll start by adding collection-level shared tests, then folder-level tests, and finally request-specific tests.
...
what i want is to fetch metadata only of any given spotify track
...
```
{
    ...
    "extensions": {
        "persistedQuery": {
            "version": 1,
            "sha256Hash": 
                "ae85b52abb74d20a4c331d4143d4772c95f34757bf
                a8c625c6a2c5ac96c75a24"
        }
    }
}
```
...
1. **Collection-level tests (shared across all requests)**
   ...
   pm.test(`Response time is under ${MAX_RESPONSE_TIME} 
       ms`, () => {
...
2. **Folder-level tests (grouped by host)**
   ...
   - **Folder `gae2-spclient.spotify.com`**
     ...
     if (pm.response.headers.get("Content-Type") && pm.
       ...
       pm.test("Body is valid JSON", () => {
         pm.expect(() => pm.response.json()).to.not.
             throw();
       });
     
       // General schema-like checks for standard 
         Spotify JSON envelopes
       pm.test("JSON has at least one top-level key", 
         () => {
         const body = pm.response.json();
         pm.expect(Object.keys(body).length).to.be.
             greaterThan(0);
       });
     }
     ```
   ...
   - **Folder `video-fa.scdn.co`** (segment/init binaries)
     ...
     pm.test("Returns binary data or MP4", () => {
       const contentType = pm.response.headers.get
         ("Content-Type") || "";
       pm.expect(
     ...
     pm.test("Segment/init response is not empty", () 
         => {
       pm.expect(pm.response.text().length).to.be.
         greaterThan(0);
     });
     ```
   ...
   - **Folder `seektables.scdn.co`** (seek table JSON)
     ...
     if (pm.response.headers.get("Content-Type")?.
       ...
       pm.test("Seek table has entries array or similar 
         structure", () => {
         const body = pm.response.json();
         pm.expect(Object.keys(body).length).to.be.
             greaterThan(0);
   ...
   - **Analytics / events folders**
     ...
     ```
     pm.test("Request accepted by analytics/event 
         endpoint", () => {
       pm.expect([200, 202, 204, 302]).to.include(pm.
         response.code);
     });
     
     pm.test("Response has expected basic body 
         characteristics", () => {
       const text = pm.response.text();
       // Some of these endpoints return empty bodies; 
         this test only asserts no runtime error
...
3. **Request-level tests (specific, based on actual responses)**
   ...
   - **`https://api-partner.spotify.com/pathfinder/v2/query`** (POST)
     ...
     pm.test("Response is valid JSON and contains track 
         data", () => {
       const json = pm.response.json();
       pm.expect(json).to.have.property("data");
       pm.expect(json.data).to.have.property("track");
       pm.expect(json.data.track).to.have.property
         ("name");
       pm.expect(json.data.track).to.have.property
         ("uri");
     });
   ...
   - **`https://gae2-spclient.spotify.com/melody/v1/time`** (GET)
     ...
     pm.test("Melody time has timestamp", () => {
       ...
       pm.expect(json).to.have.property
         ("client_timestamp").that.is.a("number");
     });
     ```
   ...
   - **`https://gae2-spclient.spotify.com/track-playback/v1/devices/.../state`** (PUT)
     ...
     pm.test("Device state update succeeded", () => {
       pm.expect([200, 202, 204]).to.include(pm.
         response.code);
     });
     ```
   ...
   - **`https://spclient.wg.spotify.com/collection/v2/contains`** (POST)
     ...
     ```
     pm.test("Collection contains endpoint returns 
         success", () => {
       pm.response.to.have.status(200);
     });
     ...
     pm.test("Contains response structure", () => {
       ...
       pm.expect(Object.keys(json).length).to.be.
         greaterThan(0);
     });
     ```
...
if (!input) {
    throw new Error("REQUEST_INPUT not set. Provide: 
        'service id', 'service query', 'ISRC', or 'track 
        name'");
}
...
// Detect query type
// Basic ISRC pattern: CC + 3 + 2 + 5, allow some 
    flexibility and ignore case
const isrcCandidate = queryValue.replace(/[^A-Za-z0-9]/g, 
    "").toUpperCase();
...
if (!service) {
    if (queryType === "isrc") {
        ...
        throw new Error("Numeric ID requires service name 
            (e.g., 'beatport 17606729')");
    } else {
...
const queryValue = pm.environment.get
    ("PARSED_QUERY_VALUE");
const token = pm.environment.get("BEATPORT_ACCESS_TOKEN");

if (!token) {
    throw new Error("BEATPORT_ACCESS_TOKEN not set in 
        environment");
}
...
if (queryType === "id") {
    ...
    url = `https://api.beatport.com/v4/catalog/search/?q=$
        {encodeURIComponent(queryValue)}&type=tracks`;
} else {
    // Text search
    url = `https://api.beatport.com/v4/catalog/search/?q=$
        {encodeURIComponent(queryValue)}&type=tracks`;
...
pm.request.headers.upsert({ key: "Authorization", value: 
    `Bearer ${token}` });
pm.request.headers.upsert({ key: "User-Agent", value: 
    "Mozilla/5.0 (Postman Dynamic Request)" });
...
const countryCode = pm.environment.get
    ("TIDAL_COUNTRY_CODE") || "US";

if (!token) {
    throw new Error("TIDAL_AUTH_TOKEN not set in 
        environment");
}
...
if (queryType === "id") {
    url = `https://openapi.tidal.com/v2/tracks/$
        {queryValue}?countryCode=${encodeURIComponent
        (countryCode)}`;
} else if (queryType === "isrc") {
    // Tidal ISRC search (using query + filter)
    url = `https://api.tidal.com/v1/search?query=$
        {encodeURIComponent(queryValue)}&types=TRACKS&
...
const clientSecret = pm.environment.get
    ("SPOTIFY_CLIENT_SECRET");
let token = pm.environment.get("SPOTIFY_ACCESS_TOKEN");

if (!clientId || !clientSecret) {
    throw new Error("SPOTIFY_CLIENT_ID or 
        SPOTIFY_CLIENT_SECRET not set in environment");
}

// Helper function to set URL and header AFTER we know the 
    token
function prepareSpotifyRequest(accessToken) {
    ...
    if (queryType === "id") {
        ...
        url = `https://api.spotify.com/v1/search?q=isrc:$
            {encodeURIComponent(queryValue)}&type=track&
            limit=10`;
    } else {
...
if (token) {
    ...
    // No token yet: fetch it synchronously with pm.
        sendRequest
    ...
    pm.sendRequest({
        ...
        if (!newToken) {
            throw new Error("Spotify token missing in 
                token response");
        }
        pm.environment.set("SPOTIFY_ACCESS_TOKEN", 
            newToken);
...
if (!token || !appId) {
    throw new Error("QOBUZ_USER_AUTH_TOKEN or QOBUZ_APP_ID 
...
if (queryType === "id") {
    url = `https://www.qobuz.com/api.json/0.2/track/get?
        track_id=${encodeURIComponent(queryValue)}&app_id=$
        {encodeURIComponent(appId)}`;
} else if (queryType === "isrc") {
    // ISRC search via 'query' parameter
    url = `https://www.qobuz.com/api.json/0.2/track/search?
        query=${encodeURIComponent(queryValue)}&app_id=$
        {encodeURIComponent(appId)}`;
} else {
    url = `https://www.qobuz.com/api.json/0.2/track/search?
        query=${encodeURIComponent(queryValue)}&app_id=$
...
pm.request.url = url;
pm.request.headers.upsert({ key: "X-User-Auth-Token", 
    value: token });
pm.request.headers.upsert({ key: "X-App-Id", value: 
    appId });
...
```
// === Shared Tests: Extract duration and store full 
    response ===
...
try {
    ...
    console.warn("Response is not JSON, skipping duration 
        extraction");
    return;
}
...
// Beatport: common pattern is either a single track or 
    search results
if (source === "beatport") {
    if (data && typeof data === "object") {
        ...
        if (data.length_ms !== undefined) {
            ...
            results.length > 0 && data.results[0].
            length_ms !== undefined) {
...
// Tidal: seconds to ms
if (source === "tidal") {
    if (data.duration !== undefined && typeof data.
        duration === "number") {
        duration_ms = data.duration * 1000;
    } else if (data.data && data.data.length && data.data
        [0].attributes) {
        // v2 style with attributes.duration (seconds) or 
            ISO 8601
        ...
        if (typeof attr.duration === "number") {
            ...
            const m = attr.duration.match(/PT(?:(\d+)H)?(?:
                (\d+)M)?(?:(\d+)S)?/);
            if (m) {
                const hours = parseInt(m[1] || "0", 10);
                const mins = parseInt(m[2] || "0", 10);
                const secs = parseInt(m[3] || "0", 10);
                duration_ms = (hours * 3600 + mins * 60 + 
                    secs) * 1000;
            }
        }
    } else if (data.items && data.items[0] && data.items
        [0].duration !== undefined) {
...
if (source === "spotify") {
    if (data.duration_ms !== undefined) {
        ...
        items) && data.tracks.items.length > 0 && data.
        tracks.items[0].duration_ms !== undefined) {
...
if (source === "qobuz") {
    if (data.duration !== undefined && typeof data.
        duration === "number") {
        duration_ms = data.duration * 1000;
    } else if (Array.isArray(data.tracks) && data.tracks
        [0] && typeof data.tracks[0].duration === 
        "number") {
...
if (duration_ms != null) {
    pm.environment.set("LAST_DURATION_MS", duration_ms.
        toString());

    pm.test("Duration extracted and greater than 0", 
        function () {
...
pm.environment.set("LAST_RESPONSE", JSON.stringify(data, 
    null, 2));
console.log("Full response:", JSON.stringify(data, null, 
    2));
```
...
SPOTIFY_TOKEN_EXPIRES_AT // epoch ms (optional, for nicer 
    refresh logic)
...
//
// OUTPUT ENV VARS:
//   PARSED_SERVICE   : "beatport" | "tidal" | "spotify" | 
    "apple" | "itunes" | "qobuz" | "multi"
...
if (!input) {
    throw new Error("REQUEST_INPUT not set. Examples: 
        'beatport 17606729', 'spotify Justin Timberlake 
        Filthy', 'USRC11702778', 'High Street'.");
}
...
const normalized = queryValue.replace(/[^A-Za-z0-9]/g, "").
    toUpperCase();
...
if (/^[A-Z]{2}[A-Z0-9]{7,10}$/.test(normalized)) {
    ...
    console.log("[Dynamic Music API] Detected ISRC:", 
        queryValue);
} else if (/^\d+$/.test(queryValue)) {
    queryType = "id";
    console.log("[Dynamic Music API] Detected numeric 
        ID:", queryValue);
} else {
    queryType = "search";
    console.log("[Dynamic Music API] Detected search 
        query:", queryValue);
}
...
if (!service) {
    if (queryType === "isrc") {
        ...
        throw new Error("Numeric ID requires a service 
            name. Example: 'beatport 17606729'");
    } else {
...
console.log("[Dynamic Music API] Query value:", 
    queryValue);
```
...
pm.request.headers.upsert({ key: "Authorization", value: 
    `Bearer ${token}` });
pm.request.headers.upsert({ key: "User-Agent", value: 
    "Mozilla/5.0 (Postman Dynamic Beatport)" });
...
if (queryType === "id") {
    url = `https://openapi.tidal.com/v2/tracks/$
        {encodeURIComponent(queryValue)}?countryCode=$
...
const clientSecret = pm.environment.get
    ("SPOTIFY_CLIENT_SECRET");
let token = pm.environment.get("SPOTIFY_ACCESS_TOKEN");
let tokenExpiresAt = parseInt(pm.environment.get
    ("SPOTIFY_TOKEN_EXPIRES_AT") || "0", 10);
...
if (token && tokenExpiresAt && now < tokenExpiresAt - 
    ...
    pm.sendRequest({
        ...
        if (err || !res.ok) {
            console.error("Failed to fetch Spotify access 
                token:", err || res.text());
            throw new Error("Failed to fetch Spotify 
                access token");
        }
        ...
        pm.environment.set("SPOTIFY_ACCESS_TOKEN", 
            accessToken);
        pm.environment.set("SPOTIFY_TOKEN_EXPIRES_AT", 
            String(expiresAt));
...
const queryValue = pm.environment.get
    ("PARSED_QUERY_VALUE");
const devToken = pm.environment.get
    ("APPLE_MUSIC_DEV_TOKEN");
const userToken = pm.environment.get
    ("APPLE_MUSIC_USER_TOKEN"); // optional
const storefront = (pm.environment.get
    ("ITUNES_COUNTRY_CODE") || "US").toLowerCase();
...
if (queryType === "id") {
    url = `https://api.music.apple.com/v1/catalog/$
        {storefront}/songs/${encodeURIComponent
        (queryValue)}`;
} else if (queryType === "isrc") {
    // Filter by ISRC
    url = `https://api.music.apple.com/v1/catalog/$
        {storefront}/songs?filter[isrc]=$
        {encodeURIComponent(queryValue)}&limit=25`;
} else {
    // Text search
    url = `https://api.music.apple.com/v1/catalog/$
        {storefront}/search?term=${encodeURIComponent
...
if (userToken) {
    headers.push({ key: "Music-User-Token", value: 
        userToken });
}
...
const queryValue = pm.environment.get
    ("PARSED_QUERY_VALUE");
const country = pm.environment.get
...
if (queryType === "id") {
    ...
    url = `https://itunes.apple.com/lookup?id=$
        {encodeURIComponent(queryValue)}&country=$
        {encodeURIComponent(country)}`;
} else if (queryType === "isrc") {
    // iTunes Search ISRC
    url = `https://itunes.apple.com/search?term=$
        {encodeURIComponent(queryValue)}&entity=song&
...
if (queryType === "id") {
    ...
    url = `https://www.qobuz.com/api.json/0.2/track/search?
        query=${encodeURIComponent(queryValue)}&app_id=$
        {encodeURIComponent(appId)}&limit=50`;
}
...
if (source === "beatport") {
    if (data.length_ms !== undefined) {
        duration_ms = data.length_ms;
    } else if (Array.isArray(data.results) && data.results
        [0]?.length_ms !== undefined) {
        duration_ms = data.results[0].length_ms;
    }
}
...
if (source === "spotify") {
    if (data.duration_ms !== undefined) {
        duration_ms = data.duration_ms;
    } else if (data.tracks?.items?.[0]?.duration_ms !== 
        undefined) {
        duration_ms = data.tracks.items[0].duration_ms;
    }
}
...
if (source === "apple") {
    // Catalog track
    if (data.data?.[0]?.attributes?.durationInMillis !== 
        undefined) {
        duration_ms = data.data[0].attributes.
            durationInMillis;
    } else if (data.results?.songs?.data?.[0]?.attributes?.
        durationInMillis !== undefined) {
        duration_ms = data.results.songs.data[0].
...
if (source === "qobuz") {
    if (typeof data.duration === "number") {
        duration_ms = data.duration * 1000;
    } else if (Array.isArray(data.tracks) && typeof data.
        tracks[0]?.duration === "number") {
        duration_ms = data.tracks[0].duration * 1000;
    }
}

if (duration_ms != null) {
    pm.environment.set("LAST_DURATION_MS", String
        (duration_ms));
...
- In a collection-level pre-request:
  ...
  ```
  const dataInput = pm.iterationData.get
      ("input_string");
  if (dataInput) {
...
try {
    ...
    console.warn("Response not JSON; skipping 
    return;
}
...
let agg = {};
try {
    const existing = pm.environment.get
        ("AGGREGATED_METADATA");
...
if (service === "spotify") {
    ...
    if (t) {
        ...
        record.artists = (t.artists || []).map(a => a.
            name).filter(Boolean);
        record.album = t.album?.name || null;
        record.duration_ms = t.duration_ms || null;
        record.explicit = t.explicit ?? null;
        record.release_date = t.album?.release_date || 
        record.album_art_url = t.album?.images?.[0]?.
            url || null;
...
if (service === "beatport") {
    ...
    if (tr) {
        ...
        record.artists = (tr.artists || tr.producers || 
        ...
        record.explicit = null; // Beatport doesn't use 
            explicit flag like streaming services
        record.album_art_url = tr.images?.large || tr.
            images?.medium || null;
    }
}
...
if (service === "tidal") {
    ...
    if (tr) {
        ...
        record.artists = (tr.artists || tr.artist?.name ? 
            [tr.artist.name] : []).map(a => (typeof a === 
            "string" ? a : a.name)).filter(Boolean);
        record.album = tr.album?.title || tr.albumTitle || 
            null;
        record.duration_ms = (tr.duration ? tr.duration * 
            1000 : null);
        record.isrc = tr.isrc || tr.externalIds?.isrc || 
        record.release_date = tr.streamStartDate || tr.
            releaseDate || null;
        record.album_art_url = tr.album?.cover || tr.
            image || null;
    }

    // If lyrics snippet present anywhere (non-standard)
    if (raw.lyrics?.lines) {
        record.lyrics_excerpt = raw.lyrics.lines.map(l => 
            l.words).join(" ");
    }
}
...
if (service === "apple") {
    ...
    if (raw.data?.length && raw.data[0].attributes) {
        tr = raw.data[0].attributes;
    } else if (raw.results?.songs?.data?.length && raw.
        results.songs.data[0].attributes) {
    ...
    if (tr) {
        ...
        // lyrics + pdf would need extra calls to private 
            endpoints; if you call them, parse into record.
            lyrics / pdf_companion_urls
    }
}
...
if (service === "itunes") {
    if (Array.isArray(raw.results) && raw.results[0]) {
        ...
        record.isrc = tr.isrc || null; // some iTunes 
            results expose ISRC
        record.release_date = tr.releaseDate || null;
        record.album_art_url = tr.artworkUrl100 || tr.
            artworkUrl60 || null;
    }
}
...
if (service === "qobuz") {
    ...
    if (tr) {
        ...
        record.artists = [tr.performer?.name || tr.artist?.
            name].filter(Boolean);
        record.album = tr.album?.title || null;
        record.duration_ms = tr.duration ? tr.duration * 
            1000 : null;
        record.isrc = tr.isrc || null;
        record.release_date = tr.release_date || tr.album?.
            release_date || null;
        record.album_art_url = tr.image?.large || tr.
            image?.thumbnail || null;
    }

    // If you also call album/get, parse PDF companion 
        URLs there:
    if (raw.digital_booklet_url) {
        record.pdf_companion_urls.push(raw.
            digital_booklet_url);
    }
    if (raw.album?.booklet_url) {
        record.pdf_companion_urls.push(raw.album.
    }
}
...
// Persist back
pm.environment.set("AGGREGATED_METADATA", JSON.stringify
        aggregation");
            null;
        {
            null;
...
# Optional: share a token / expiry if you cache it from 
    Postman or from a script
...
# Apple Music / iTunes
export 
    APPLE_MUSIC_DEV_TOKEN="REPLACE_WITH_APPLE_MUSIC_DEV_JWT
    "
export 
    APPLE_MUSIC_USER_TOKEN="REPLACE_WITH_APPLE_MUSIC_USER_T
    OKEN"   # optional
export ITUNES_COUNTRY_CODE="US"

# Qobuz
export 
    QOBUZ_USER_AUTH_TOKEN="REPLACE_WITH_QOBUZ_USER_TOKEN"
...
postman collection run dynamic-music-api.
...
`/Users/georgeskhawam/Projects/dedupe/info.txt` is essentially a scratchpad that already contains:
...
QOBUZ_SECRET_3 = abb21364945c0583309667d13ca7d93a  # check 
    typo from file
...
export BEATPORT_ACCESS_TOKEN="REPLACE_WITH_BEATPORT_TOKEN"
export 
    BEATPORT_ACCESS_TOKEN_ALT="REPLACE_WITH_BEATPORT_ALT"
...
export 
    APPLE_MUSIC_USER_TOKEN="REPLACE_APPLE_MUSIC_USER_TOKEN"
export ITUNES_COUNTRY_CODE="US"
```
...
```
// No need for pre-request here unless you want to 
    dynamically pick token.
```
...
if (data.access_token) {
    pm.environment.set("TIDAL_ACCESS_TOKEN", data.
        access_token);
}
if (data.refresh_token) {
    pm.environment.set("TIDAL_REFRESH_TOKEN", data.
        refresh_token);
}
if (data.expires_in) {
    const expiresAt = Date.now() + data.expires_in * 1000;
    pm.environment.set("TIDAL_TOKEN_EXPIRES_AT", String
        (expiresAt));
}

pm.test("Tidal token refreshed", () => {
    pm.expect(pm.environment.get("TIDAL_ACCESS_TOKEN")).to.
        exist;
});
```
...
```
const now = Date.now();
const expiresAt = parseInt(pm.environment.get
    ("TIDAL_TOKEN_EXPIRES_AT") || "0", 10);
const accessToken = pm.environment.get
    ("TIDAL_ACCESS_TOKEN");
const refreshToken = pm.environment.get
    ("TIDAL_REFRESH_TOKEN");

if (!accessToken && !refreshToken) {
    throw new Error("No Tidal access or refresh token 
}

// Simple approach: assume you refresh manually.
// If you want auto-refresh, you can call pm.sendRequest() 
        set");
    to the token endpoint, similar to Spotify.
```
...
if (queryType === "id") {
    url = `https://api.tidalhifi.com/v1/tracks/$
        {encodeURIComponent(queryValue)}?countryCode=$
        {encodeURIComponent(countryCode)}&include=lyrics`;
} else {
    url = `https://api.tidalhifi.com/v1/search?query=$
        {encodeURIComponent(queryValue)}&types=TRACKS&
        countryCode=${encodeURIComponent(countryCode)}&
        limit=50`;
}
...
try {
    ...
    console.warn("Qobuz response not JSON, skipping 
        booklet fetch");
    return;
}

// Get album id
let albumId = trackJson?.album?.id || (Array.isArray
    (trackJson.tracks) && trackJson.tracks[0]?.album?.id);
if (!albumId) {
    console.warn("No album id found in Qobuz track 
        response, skipping booklet fetch");
    return;
}

const appId = pm.environment.get("QOBUZ_APP_ID");
if (!appId) {
    console.warn("QOBUZ_APP_ID not set, cannot fetch album 
        booklet");
    return;
}
...
pm.sendRequest({
    ...
    if (err || !res.ok) {
        console.warn("Failed to fetch Qobuz album metadata 
            for booklet", err || res.text());
        return;
    }
    ...
    const urls = [];
    if (albumData.digital_booklet_url) urls.push(albumData.
        digital_booklet_url);
    if (albumData.booklet_url) urls.push(albumData.
        booklet_url);
    if (albumData.pdf_url) urls.push(albumData.pdf_url);
    if (Array.isArray(albumData.assets)) {
        albumData.assets.forEach(a => {
            if (a.url && /\.pdf(\?|$)/i.test(a.url)) urls.
                push(a.url);
        });
    }
    ...
    // Merge these into AGGREGATED_METADATA.qobuz.
        pdf_companion_urls
    let agg = {};
    try {
        const existing = pm.environment.get
    ...
    const existingUrls = new Set(agg.qobuz.
        pdf_companion_urls || []);
    urls.forEach(u => existingUrls.add(u));

    agg.qobuz.pdf_companion_urls = Array.from

    pm.environment.set("AGGREGATED_METADATA", JSON.
        stringify(agg, null, 2));
        (existingUrls);
});
```
...
if (service === "tidal") {
    ...
    if (raw.lyrics) {
        if (Array.isArray(raw.lyrics.lines)) {
            record.lyrics = raw.lyrics.lines.map(l => l.
                words).join("\n");
            record.lyrics_excerpt = record.lyrics.slice(0, 
...
const parsedValue = pm.environment.get
    ("PARSED_QUERY_VALUE");
const lastDuration = pm.environment.get
    ("LAST_DURATION_MS");
let agg = {};
...
const exportRecord = {
    ...
    last_duration_ms: lastDuration ? Number(lastDuration) 
        : null,
    aggregated: agg,
...
pm.variables.set("EXPORT_RECORD", JSON.stringify
    (exportRecord));
...
pm.request.headers.upsert({ key: "Content-Type", value: 
    "application/json" });
```
...
OUTPUT_FILE = "exported_metadata.ndjson"  # one JSON per 
    line
...
if __name__ == "__main__":
    ...
    print(f"Listening on http://127.0.0.1:{port}, writing 
        to {OUTPUT_FILE}")
...
3. Follow the steps to connect your GitHub repo (`dedupe` or whichever you want):
...
CREATE TABLE tracks (
  ...
  library_track_id TEXT UNIQUE,          -- your own local 
    ID or hash (optional but recommended)
  title           TEXT,
  artist          TEXT,
  album           TEXT,
  duration_ms     INTEGER,
...
CREATE TABLE track_sources (
  id              INTEGER PRIMARY KEY AUTOINCREMENT,
  library_track_id TEXT,          -- FK to tracks.
    library_track_id
  service         TEXT,           -- 'spotify','tidal',
    'qobuz','beatport','apple','itunes'
  service_track_id TEXT,          -- per-service ID
  url             TEXT,           -- canonical URL if you 
    want
  metadata_json   TEXT,           -- full raw per-service 
    JSON (optional)
  duration_ms     INTEGER,
  isrc            TEXT,
  album_art_url   TEXT,
...
```
{
    ...
    "summary": {
        ...
        "lyrics_excerpt": "First 500 chars from any 
            service that has lyrics"
    },
    "sources": [
        {
            "service": "spotify",
            ...
            "pdf_companions": [],
            "lyrics_excerpt": null,
            "metadata_json": { /* the full aggregated.
                spotify.raw */}
        },
        {
            "service": "tidal",
            ...
            "duration_ms": 234000,
            "isrc": "USRC11702778",
            "album_art_url": "https://resources.tidal.com/
                images/...",
            "pdf_companions": [],
        ...
        },
        {
            ...
            "duration_ms": 234000,
            "isrc": "USRC11702778",
            "album_art_url": "https://static.qobuz.com/
                images/...",
            "pdf_companions": [
            "lyrics_excerpt": null,
...
let libraryTrackId = pm.iterationData.get
    ("library_track_id");
if (!libraryTrackId) {
    // Fallback: use REQUEST_INPUT as a synthetic id, or 
        leave null
    libraryTrackId = null;
}
...
function addSource(name, rec) {
    ...
    // Build a canonical "service_track_id" & url where 
        possible
    ...
    if (name === "spotify") {
        ...
        else if (raw.tracks?.items?.[0]?.id) 
            serviceTrackId = raw.tracks.items[0].id;
        if (serviceTrackId) {
            url = `https://open.spotify.com/track/$
                {serviceTrackId}`;
        }
        ...
        else if (raw.items?.[0]?.item?.id) serviceTrackId 
            = String(raw.items[0].item.id);
        // url patterns vary; you can add them if needed
    } else if (name === "qobuz") {
        const raw = rec.raw || {};
        if (raw.id) serviceTrackId = String(raw.id);
        else if (raw.tracks?.[0]?.id) serviceTrackId = 
            String(raw.tracks[0].id);
    } else if (name === "beatport") {
        const raw = rec.raw || {};
        if (raw.id) serviceTrackId = String(raw.id);
        else if (raw.results?.[0]?.id) serviceTrackId = 
            String(raw.results[0].id);
        if (serviceTrackId) {
        ...
        const raw = rec.raw || {};
        if (raw.data?.[0]?.id) serviceTrackId = raw.data
    } else if (name === "itunes") {
        const raw = rec.raw || {};
        if (raw.results?.[0]?.trackId) serviceTrackId = 
            String(raw.results[0].trackId);
    }
    ...
    // Build "best" summary: prefer Spotify, then Tidal, 
        then Qobuz, then Beatport, then Apple/iTunes
    const priority = ["spotify", "tidal", "qobuz", 
        "beatport", "apple", "itunes"];
    const currentScore = priority.indexOf(summaryBest.
        _source || "zzz");
    const newScore = priority.indexOf(name);
    const betterSource = (newScore !== -1 && (currentScore 
        === -1 || newScore < currentScore));

    if (betterSource) {
        summaryBest = {
            _source: name,
            title: rec.title || null,
            artist: (rec.artists && rec.artists.length ? 
                rec.artists.join(", ") : null),
        ...
        if (!summaryBest.title && rec.title) summaryBest.
            title = rec.title;
        if (!summaryBest.artist && rec.artists?.length) 
            summaryBest.artist = rec.artists.join(", ");
        if (!summaryBest.album && rec.album) summaryBest.
            album = rec.album;
        if (!summaryBest.duration_ms && rec.duration_ms) 
            summaryBest.duration_ms = rec.duration_ms;
        if (!summaryBest.isrc && rec.isrc) summaryBest.
            isrc = rec.isrc;
        if (!summaryBest.release_date && rec.release_date) 
            summaryBest.release_date = rec.release_date;
        if (summaryBest.explicit === null && rec.explicit 
            !== undefined) summaryBest.explicit = rec.
        if (!summaryBest.best_cover_url && rec.
            album_art_url) summaryBest.best_cover_url = 
        if (!summaryBest.lyrics_excerpt && rec.
...
console.log("[EXPORT_RECORD_DB]", pm.variables.get
    ("EXPORT_RECORD"));
```
...
1. Parse line-by-line in Python and insert:
   ...
   with open("exported_metadata.ndjson", 
       encoding="utf-8") as f:
       for line in f:
           if not line.strip():
               continue
           ...
           if not lib_id:
               # decide how to generate a stable id, e.
                   g., hash of ISRC+title
               lib_id = rec["summary"].get("isrc") or rec
                   ["request_input"]
   
           s = rec["summary"] or {}
           cur.execute("""
             INSERT OR REPLACE INTO tracks
               (library_track_id, title, artist, album, 
                   duration_ms, isrc,
                release_date, explicit, best_cover_url, 
                   lyrics_excerpt)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
           """, (
             lib_id,
           ...
           for src in rec["sources"]:
               cur.execute("""
                 INSERT INTO track_sources
                   (library_track_id, service, 
                       service_track_id, url,
                    metadata_json, duration_ms, isrc, 
                       album_art_url,
...
the db is here: /Users/georgeskhawam/Projects/dedupe/EPOCH_2026-01-24/music.db

Got it: your SQLite DB is `/Users/georgeskhawam/Projects/dedupe/EPOCH_2026-01-24/music.db`.
...
```
cd /Users/georgeskhawam/Projects/dedupe/EPOCH_2026-01-24
...
Run in `sqlite3 /Users/georgeskhawam/Projects/dedupe/EPOCH_2026-01-24/music.db`:

```
-- Main logical track table
CREATE TABLE IF NOT EXISTS library_tracks (
  id              INTEGER PRIMARY KEY AUTOINCREMENT,
  library_track_key TEXT UNIQUE,      -- stable key: ISRC 
    or your own
  title           TEXT,
  artist          TEXT,
  album           TEXT,
...
CREATE INDEX IF NOT EXISTS idx_library_tracks_isrc ON 
    library_tracks(isrc);
CREATE INDEX IF NOT EXISTS idx_library_tracks_title_artist 
    ON library_tracks(title, artist);

-- Per-service metadata (spotify/tidal/qobuz/beatport/
    apple/itunes)
CREATE TABLE IF NOT EXISTS library_track_sources (
  ...
  lyrics_excerpt  TEXT,
  fetched_at      TEXT DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY(library_track_key) REFERENCES library_tracks
    (library_track_key)
);
...
CREATE INDEX IF NOT EXISTS 
    idx_library_track_sources_service ON 
    library_track_sources(service);
```
...
// 2) Else, if we have a clear ISRC in summary later, use 
    that.
// 3) Else, fall back to request_input.
let explicitLibraryKey = pm.iterationData.get
    ("library_track_key") || null;
...
// We'll build per-service sources first, and also a 
    "best" summary.
const sources = [];
let summaryBest = {
    _source: null,
...
function addSource(name, rec) {
    ...
    if (name === "spotify") {
        if (raw.id) serviceTrackId = raw.id;
            ...
            serviceTrackId = raw.tracks.items[0].id;
        if (serviceTrackId) url = `https://open.spotify.
            com/track/${serviceTrackId}`;
    } else if (name === "tidal") {
        if (raw.id) serviceTrackId = String(raw.id);
        else if (raw.items?.[0]?.item?.id) serviceTrackId 
            = String(raw.items[0].item.id);
        ...
        if (raw.id) serviceTrackId = String(raw.id);
            ...
            String(raw.results[0].id);
        if (serviceTrackId) url = `https://www.beatport.
            com/track/-/${serviceTrackId}`;
    } else if (name === "apple") {
        if (raw.data?.[0]?.id) serviceTrackId = raw.data
    } else if (name === "itunes") {
    ...
    // Choose the "best" summary source according to a 
        priority.
    const priority = ["spotify", "tidal", "qobuz", 
        "beatport", "apple", "itunes"];
    const currentScore = priority.indexOf(summaryBest.
...
DB_PATH = "/Users/georgeskhawam/Projects/dedupe/
    EPOCH_2026-01-24/music.db"
NDJSON_PATH = "/Users/georgeskhawam/Projects/dedupe/
    EPOCH_2026-01-24/exported_metadata.ndjson"
...
with Path(NDJSON_PATH).open(encoding="utf-8") as f:
    for line in f:
        ...
        # Upsert into library_tracks (simple strategy: 
            DELETE+INSERT or INSERT OR REPLACE)
        cur.execute("""
          INSERT OR REPLACE INTO library_tracks
            (library_track_key, title, artist, album, 
                duration_ms,
             isrc, release_date, explicit, best_cover_url, 
                lyrics_excerpt)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
          key,
        ...
        # Insert sources (you may want to clean old ones 
            for this key first)
        # Example: delete and re-insert each run
        cur.execute("DELETE FROM library_track_sources 
            WHERE library_track_key = ?", (key,))

        for src in rec.get("sources", []):
            cur.execute("""
              INSERT INTO library_track_sources
                (library_track_key, service, 
                    service_track_id, url,
                 metadata_json, duration_ms, isrc, 
                    album_art_url,
                 pdf_companions, lyrics_excerpt)
...
ALTER TABLE library_track_sources ADD COLUMN musical_key 
    TEXT;
ALTER TABLE library_track_sources ADD COLUMN album_title 
    TEXT;
ALTER TABLE library_track_sources ADD COLUMN artist_name 
    TEXT;
ALTER TABLE library_track_sources ADD COLUMN track_number 
    INTEGER;
ALTER TABLE library_track_sources ADD COLUMN disc_number 
    INTEGER;
```
...
if (service === "beatport") {
    ...
    if (tr) {
        ...
        record.disc_number = null;  // Beatport usually 
            doesn’t have discs.
    }
}
```
...
if (service === "spotify") {
    ...
    if (t) {
        ...
        // Genres usually live on the artist or album, but 
            not on track; skipping direct mapping here 
            unless you call artist API.
        record.genre = null;
        record.bpm = null; // audio-features is 403 for 
...
const src = {
    ...
    artist_name: (rec.artists && rec.artists.length ? rec.
        artists.join(", ") : null),
...
summaryBest = {
    _source: name,
    title: rec.title || null,
    artist: (rec.artists && rec.artists.length ? rec.
        artists.join(", ") : null),
    album: rec.album || null,
    duration_ms: rec.duration_ms ?? null,
    isrc: rec.isrc || null,
    ...
    title: summaryBest.title,
        artist: summaryBest.artist,
            album: summaryBest.album,
                duration_ms: summaryBest.duration_ms,
                    isrc: summaryBest.isrc,
                        release_date: summaryBest.
                            release_date,
                            explicit: summaryBest.
                                explicit ? 1 : 0,
                                best_cover_url: 
                                    summaryBest.
                                    best_cover_url,
...
In `sqlite3 /Users/georgeskhawam/Projects/dedupe/EPOCH_2026-01-24/music.db`:

```
ALTER TABLE files ADD COLUMN canonical_duration REAL;
ALTER TABLE files ADD COLUMN canonical_duration_source 
    TEXT;
...
const mode = pm.environment.get("MODE") || "recovery"; // 
    'recovery' or 'hoarding'
const path = pm.iterationData.get("path");
const dbDuration = pm.iterationData.get("duration"); // 
    seconds from db

// AGGREGATED_METADATA built by earlier steps (service 
    calls)
let agg = {};
try {
    agg = JSON.parse(pm.environment.get
        ("AGGREGATED_METADATA") || "{}");
} catch (e) {
    agg = {};
}
...
function toMs(val, unit) {
    ...
    if (unit === "s" || unit === "seconds") return Math.
        round(parseFloat(val) * 1000);
...
function addProvider(name, rec, unit) {
    if (!rec) return;
    providers[name] = {
        duration_ms: toMs(rec.duration_ms ?? rec.
            duration ?? null, unit),
        title: rec.title || null,
        artist: rec.artists && rec.artists.length ? rec.
            artists.join(", ") : null,
...
const precedence = ["qobuz", "tidal", "apple", "itunes", 
    "spotify", "beatport", "musicbrainz"];
let canonical = null;

for (const p of precedence) {
    const info = providers[p];
    if (info && info.duration_ms) {
        canonical = { source: p, duration_ms: info.
            duration_ms };
        break;
    }
}
...
const exportRecord = {
    mode,
    path,
    db_duration: dbDuration != null ? parseFloat
        (dbDuration) : null,
    providers,
    canonical,
    timestamp_ms: Date.now()
...
DB_PATH = "/Users/georgeskhawam/Projects/dedupe/
    EPOCH_2026-01-24/music.db"
NDJSON_PATH = "/Users/georgeskhawam/Projects/dedupe/
    EPOCH_2026-01-24/recovery_export.ndjson"
...
def classify_health(db_dur, canon_dur):
  ...
  if abs(delta) <= TOLERANCE_SEC:
    return "ok", f"db={db_dur:.3f}s, canonical={canon_dur:.
        3f}s, delta={delta:.3f}s"
  elif delta < 0:  # db shorter than canonical
    return "suspect_truncated", f"db={db_dur:.3f}s < 
        canonical={canon_dur:.3f}s (delta={delta:.3f}s)"
  else:  # db longer
    return "suspect_extended", f"db={db_dur:.3f}s > 
        canonical={canon_dur:.3f}s (delta={delta:.3f}s)"
...
with Path(NDJSON_PATH).open(encoding="utf-8") as f:
  for line in f:
    ...
    canon_sec = canon_ms / 1000.0 if canon_ms is not None 
        else None
...
```
SELECT path, duration, canonical_duration, 
    metadata_health, metadata_health_reason
FROM files
...
```
ALTER TABLE files ADD COLUMN canonical_duration 
    REAL;            -- in seconds
ALTER TABLE files ADD COLUMN canonical_duration_source 
    TEXT;     -- 'spotify', 'qobuz', etc.
ALTER TABLE files ADD COLUMN metadata_health 
    TEXT;               -- 'ok','suspect_truncated',
    'suspect_extended','mismatch','unknown'
ALTER TABLE files ADD COLUMN metadata_health_reason 
    TEXT;        -- short diagnostic string
```
...
CREATE TABLE IF NOT EXISTS library_tracks (
  id                    INTEGER PRIMARY KEY AUTOINCREMENT,
  library_track_key     TEXT UNIQUE,   -- stable key based 
    on ISRC (or fallback hash)
  title                 TEXT,
  artist                TEXT,
  album                 TEXT,
  duration_ms           INTEGER,
  ...
  best_cover_url        TEXT,
  lyrics_excerpt        TEXT,          -- short text for 
    quick reference
  genre                 TEXT,
  bpm                   REAL,
  musical_key           TEXT,
...
CREATE TABLE IF NOT EXISTS library_track_sources (
  id                    INTEGER PRIMARY KEY AUTOINCREMENT,
  library_track_key     TEXT,            -- FK to logical 
    track
  service               TEXT,            -- 'spotify',
    'tidal','qobuz','beatport','apple','itunes',
    'musicbrainz','discogs'
  service_track_id      TEXT,
  url                   TEXT,
  metadata_json         TEXT,            -- full raw JSON 
    from provider
  duration_ms           INTEGER,
  isrc                  TEXT,
  album_art_url         TEXT,
  pdf_companions        TEXT,            -- JSON array of 
    URLs
  lyrics_excerpt        TEXT,
  genre                 TEXT,
  bpm                   REAL,
...
DB_PATH = "/Users/georgeskhawam/Projects/dedupe/
    EPOCH_2026-01-24/music.db"
OUT_CSV = "/Users/georgeskhawam/Projects/dedupe/
    EPOCH_2026-01-24/recovery_input.csv"
...
export 
    SPOTIFY_CLIENT_SECRET="91cceb8042624f2ea5c4420adbc28334
    "
# and others you already use: QOBUZ_APP_ID, 
    QOBUZ_APP_SECRET, TIDAL_TOKEN, etc.
```
...
```
// This script runs after all provider calls, e.g. on 
    "Aggregate Results" request.
// It reads per-request data that you've stored in 
    environment variables or globals.
...
//   env.S_RAW, env.Q_RAW, env.T_RAW, env.A_RAW, env.
    I_RAW, env.BP_RAW, env.MB_RAW
...
function normalizeSpotify(s) {
    ...
    return {
        ...
        raw: s,
        title: t.name || null,
        artists: (t.artists || []).map(a => a.name).filter
            (Boolean),
...
function normalizeQobuz(q) {
    ...
    return {
        ...
        duration_ms: seconds != null ? seconds * 1000 : 
        explicit: null,
        isrc: t.isrc || null,
        release_date: t.release_date || t.album?.
            release_date || null,
        album_art_url: (t.image?.large || t.image?.
            medium || t.image?.small) || null,
        track_number: t.track_number ?? null,
        disc_number: t.media_number ?? null,
        genre: t.genre?.name || null,
        bpm: null,
        musical_key: null,
        pdf_companion_urls: t.digital_booklet_url ? [t.
            digital_booklet_url] : [],
        lyrics_excerpt: null
    };
}
...
// For brevity, I’ll just sketch signatures; you can 
    expand with your recon knowledge.

function normalizeTidal(t) { /* as in earlier answer, set 
    duration_ms = t.duration * 1000, etc. */ }
function normalizeApple(a) { /* catalog or iTunes */ }
function normalizeBeatport(bp) { /* uses bpm, key, genre, 
    etc. */ }
function normalizeMusicBrainz(mb) { /* uses length, title, 
    artists, isrcs, etc. */ }
...
const mode = pm.environment.get("MODE") || "recovery";
const path = pm.iterationData.get("path");
const dbDuration = pm.iterationData.get("duration"); // in 
    seconds
...
class Handler(BaseHTTPRequestHandler):
  def do_POST(self):
    ...
    # Append to NDJSON
    Path(NDJSON_PATH).parent.mkdir(parents=True, 
        exist_ok=True)
...
{
    "mode": "hoarding",
    "track_key": "USRC11702778", // or other stable 
        identifier
...
let agg = {};
try { agg = JSON.parse(pm.environment.get
    ("AGGREGATED_METADATA") || "{}"); } catch (e) { agg = 
    {}; }

const servicesOrder = ["qobuz", "tidal", "beatport", 
    "apple", "itunes", "spotify", "musicbrainz"];
...
const recs = servicesOrder.map(s => agg[s]).filter
    (Boolean);

// Choose track_key: prefer ISRC, else construct from 
    artist+title
...
if (!trackKey) {
    ...
    if (artist.length && title) {
        trackKey = `${artist.join(", ")}::${title}`.
            toLowerCase();
    } else {
        trackKey = `unknown::${pm.iterationData.get
            ("path") || Date.now()}`;
    }
}
...
const summary = {
    ...
    best_cover_url: pickNonEmpty("album_art_url", recs, 
        null),
    lyrics_excerpt: pickNonEmpty("lyrics_excerpt", recs, 
        null),
...
function makeSource(rec) {
    ...
    return {
        ...
        musical_key: rec.musical_key || null,
        album_title: rec.album || null,
        artist_name: (rec.artists && rec.artists.length ? 
            rec.artists.join(", ") : null),
        track_number: rec.track_number ?? null,
        disc_number: rec.disc_number ?? null,
        metadata_json: rec.raw || {}
...
DB_PATH = "/Users/georgeskhawam/Projects/dedupe/
    EPOCH_2026-01-24/music.db"
NDJSON_PATH = "/Users/georgeskhawam/Projects/dedupe/
    EPOCH_2026-01-24/hoarding_export.ndjson"
...
with Path(NDJSON_PATH).open(encoding="utf-8") as f:
  for line in f:
    ...
    for src in sources:
      cur.execute("""
        INSERT INTO library_track_sources
          (library_track_key, service, service_track_id, 
            url,
           metadata_json, duration_ms, isrc, album_art_url,
           pdf_companions, lyrics_excerpt,
           genre, bpm, musical_key,
           album_title, artist_name, track_number, 
            disc_number)
        VALUES 
            (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
            , ?)
...
def extract_tidal_track_info(tidal_json):
    ...
    duration_s = None
    # your JSON might have duration in seconds or PT 
        format; adapt as needed:
...
```
(
  tidal_id, tidal_bpm, tidal_key, tidal_genre, 
    tidal_duration_s
...
/path/to/file1.flac,215.3,12345,17606729,260231933,
...
```
# Old:
# tail -n +2 "$INPUT" | while IFS=, read -r tidal_id 
    beatport_id qobuz_id spotify_id; do

# New:
tail -n +2 "$INPUT" | while IFS=, read -r path db_duration 
    tidal_id beatport_id qobuz_id spotify_id; do
...
DB_PATH = "/Users/georgeskhawam/Projects/dedupe/
    EPOCH_2026-01-24/music.db"
CANONICAL_CSV = "/Users/georgeskhawam/Projects/dedupe/
    EPOCH_2026-01-24/metadata_canonical.csv"
TOLERANCE_SEC = 2.0

def choose_canonical_duration(row):
    # Precedence similar to earlier: Beatport > Qobuz > 
        TIDAL > Spotify
    for key in ["beatport_duration_s", "qobuz_duration_s", 
        "tidal_duration_s", "spotify_duration_s"]:
        val = row.get(key)
        if val:
            try:
                return float(val), key.split("_")[0].
                    replace("duration", "")
            except ValueError:
                continue
    return None, None
...
def classify_health(db_dur, canon_dur):
    ...
    delta = db_dur - canon_dur
    if abs(delta) <= TOLERANCE_SEC:
        return "ok", f"db={db_dur:.3f}s, canonical=
            {canon_dur:.3f}s, delta={delta:.3f}s"
    elif delta < 0:
        return "suspect_truncated", f"db={db_dur:.3f}s < 
            canonical={canon_dur:.3f}s (delta={delta:.3f}s)
            "
...
with Path(CANONICAL_CSV).open(newline="", 
    encoding="utf-8") as f:
    ...
    for row in reader:
        ...
        if not path:
            continue
        try:
            db_duration = float(row["db_duration"]) if row.
                get("db_duration") else None
        except ValueError:
            db_duration = None

        canon_dur, source = choose_canonical_duration(row)

        status, reason = classify_health(db_duration, 
            canon_dur)
...
const service = pm.environment.get("PARSED_SERVICE") || 
    "unknown";
// load agg
// record = { service, title, artists, album, duration_ms, 
    isrc, ... }
// service-specific extractors: spotify, beatport, tidal, 
    apple, itunes, qobuz
// agg[service] = record;
...
1. **Storage layer: SQLite database**
   - Located at:
     `/Users/georgeskhawam/Projects/dedupe/EPOCH_2026-01-24/music.db`
   - Contains:
...
```
ALTER TABLE files ADD COLUMN canonical_duration 
    REAL;            -- canonical duration in seconds
ALTER TABLE files ADD COLUMN canonical_duration_source 
    TEXT;     -- e.g. 'spotify','qobuz','tidal','beatport'
ALTER TABLE files ADD COLUMN metadata_health 
    TEXT;               -- 'ok','suspect_truncated',
    'suspect_extended','mismatch','unknown'
ALTER TABLE files ADD COLUMN metadata_health_reason 
    TEXT;        -- diagnostic string explaining why
```
...
CREATE TABLE IF NOT EXISTS library_tracks (
  id                    INTEGER PRIMARY KEY AUTOINCREMENT,
  library_track_key     TEXT UNIQUE,   -- stable key for 
    track identity (ISRC, or fallback)
  title                 TEXT,
  artist                TEXT,
  album                 TEXT,
  duration_ms           INTEGER,
  isrc                  TEXT,
  release_date          TEXT,          -- "YYYY-MM-DD" if 
    available
  explicit              INTEGER,       -- 0/1
  best_cover_url        TEXT,
  lyrics_excerpt        TEXT,          -- short snippet, 
    not necessary to store full lyrics
  genre                 TEXT,
  bpm                   REAL,
  musical_key           TEXT,
  created_at            TEXT DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS library_track_sources (
  id                    INTEGER PRIMARY KEY AUTOINCREMENT,
  library_track_key     TEXT,            -- foreign key to 
    library_tracks
  service               TEXT,            -- 'spotify',
    'tidal','qobuz','beatport','apple','itunes',
  ...
  url                   TEXT,
  metadata_json         TEXT,            -- full raw JSON 
    from provider as string
  duration_ms           INTEGER,
  isrc                  TEXT,
  album_art_url         TEXT,
  pdf_companions        TEXT,            -- JSON array of 
    URLs for digital booklets
  lyrics_excerpt        TEXT,
  genre                 TEXT,
  bpm                   REAL,
  musical_key           TEXT,
...
export QOBUZ_EMAIL_OR_USERID=""             # your Qobuz 
    login
export QOBUZ_PASSWORD_MD5_OR_TOKEN=""       # MD5
    (password) or auth token
export QOBUZ_APP_ID=""                      # e.g. 
    "798273057"
...
export QOBUZ_USER_AUTH_TOKEN=""             # for 
    X-User-Auth-Token
export QOBUZ_SESSION_ID=""                  # for 
...
export BEATPORT_ACCESS_TOKEN="$
    {BEATPORT_ACCESS_TOKEN_MY:-$BEATPORT_ACCESS_TOKEN_TRACK
    }"

########## SPOTIFY – CLIENT CREDENTIALS & TOKENS ##########
export 
    SPOTIFY_CLIENT_ID="9d833039ed2743948b4fbbca824bc815"   
    # your real provided ID
export 
    SPOTIFY_CLIENT_SECRET="91cceb8042624f2ea5c4420adbc28334
    " # your real provided secret
...
########## APPLE / ITUNES ##########
export APPLE_MUSIC_DEV_TOKEN=""           # JWT dev token 
    for Apple Music
...
export RETRY_BASE_DELAY="10"     # seconds before first 
    retry
...
export MUSIC_DB="/Users/georgeskhawam/Projects/dedupe/
    EPOCH_2026-01-24/music.db"
```
...
DB_PATH = os.environ.get("MUSIC_DB", "/Users/georgeskhawam/
    Projects/dedupe/EPOCH_2026-01-24/music.db")
...
def main():
    ...
    # Adjust WHERE clause based on your schema; assume 
        'deleted' or similar column exists.
    rows = cur.execute("""
      SELECT path, duration
      FROM files
      WHERE 1 = 1
    ...
    with open(OUT_CSV, "w", newline="", encoding="utf-8") 
        as f:
        w = csv.writer(f)
        # Include columns for path, db_duration, and 
            placeholder service IDs.
        w.writerow(["path", "db_duration", "tidal_id", 
            "beatport_id", "qobuz_id", "spotify_id"])
        for path, duration in rows:
            # For now, no service IDs; fill them later if 
                you have them.
...
log() {
  echo "[$(date '+%Y-%m-%dT%H:%M:%S%z')] $*" | tee -a 
    "$LOG_FILE"
}
...
do_with_retries() {
  ...
  while true; do
    ...
    log "Command failed (attempt $retries), retrying in $
        {delay}s..."
    sleep "$delay"
    delay=$((delay * 2))
  done
...
get_tidal_full() {
  ...
  if [ -z "${TIDAL_ACCESS_TOKEN:-}" ] || [ -z "$
    {TIDAL_COUNTRY_CODE:-}" ]; then
    log "WARN: TIDAL_ACCESS_TOKEN or TIDAL_COUNTRY_CODE 
        missing; skipping TIDAL for $track_id"
    echo "{}"
    return 0
  fi

  local url="https://openapi.tidal.com/v2/tracks/$
    {track_id}?countryCode=${TIDAL_COUNTRY_CODE}&
    include=lyrics"
  local cmd="curl -s -w '\n%{http_code}' \
  ...
  local response
  response=$(do_with_retries "$cmd") || { echo "{}"; 
    return 0; }
  ...
  if [ "$status" = "401" ] || [ "$status" = "403" ]; then
    log "WARN: TIDAL token invalid (status $status) for 
        track $track_id"
    echo "{\"error\":\"TIDAL_AUTH\",
        \"http_status\":$status}"
    return 0
  fi
  if [ "$status" -ge 400 ]; then
    log "WARN: TIDAL returned HTTP $status for track 
        $track_id"
    echo "{\"error\":\"TIDAL_HTTP_${status}\",
        \"http_status\":$status}"
    return 0
  fi
...
get_beatport_full() {
  ...
  local token="${BEATPORT_ACCESS_TOKEN_MY:-$
    {BEATPORT_ACCESS_TOKEN_TRACK:-}}"
  if [ -z "$token" ]; then
    log "WARN: No Beatport token set; skipping Beatport 
        for $track_id"
    echo "{}"
    return 0
  fi

  local url="https://api.beatport.com/v4/catalog/tracks/$
    {track_id}/"
  local cmd="curl -s -w '\n%{http_code}' \
  ...
  if [ "$status" = "401" ] || [ "$status" = "403" ]; then
    log "WARN: Beatport token invalid (status $status) for 
        track $track_id"
    echo "{\"error\":\"BEATPORT_AUTH\",
        \"http_status\":$status}"
    return 0
  fi
  if [ "$status" -ge 400 ]; then
    log "WARN: Beatport returned HTTP $status for track 
        $track_id"
    echo "{\"error\":\"BEATPORT_HTTP_${status}\",
        \"http_status\":$status}"
    return 0
  fi
...
get_qobuz_full() {
  ...
  if [ -z "${QOBUZ_USER_AUTH_TOKEN:-}" ] || [ -z "$
    {QOBUZ_APP_ID:-}" ]; then
    log "WARN: QOBUZ_USER_AUTH_TOKEN or QOBUZ_APP_ID 
        missing; skipping Qobuz for $track_id"
    echo "{}"
    return 0
  fi

  local base="https://www.qobuz.com/api.json/0.2/track/get"
  local url="${base}?track_id=${track_id}&app_id=$
    {QOBUZ_APP_ID}&user_auth_token=${QOBUZ_USER_AUTH_TOKEN}
    "

  local headers="-H 'Accept: */*' -H 'X-App-Id: $
    {QOBUZ_APP_ID}'"
  if [ -n "${QOBUZ_SESSION_ID:-}" ]; then
    headers="${headers} -H 'X-Session-Id: $
        {QOBUZ_SESSION_ID}'"
  fi
  ...
  if [ "$status" = "401" ] || [ "$status" = "403" ]; then
    log "WARN: Qobuz token invalid (status $status) for 
        track $track_id"
    echo "{\"error\":\"QOBUZ_AUTH\",
        \"http_status\":$status}"
    return 0
  fi
  if [ "$status" -ge 400 ]; then
    log "WARN: Qobuz returned HTTP $status for track 
        $track_id"
    echo "{\"error\":\"QOBUZ_HTTP_${status}\",
        \"http_status\":$status}"
    return 0
  fi
...
get_spotify_full() {
  ...
  if [ -z "${SPOTIFY_ACCESS_TOKEN:-}" ]; then
    log "WARN: SPOTIFY_ACCESS_TOKEN missing; skipping 
        Spotify for $track_id"
    echo "{}"
    return 0
  fi
  ...
  if [ "$status" = "401" ] || [ "$status" = "403" ]; then
    log "WARN: Spotify token invalid (status $status) for 
        track $track_id"
    echo "{\"error\":\"SPOTIFY_AUTH\",
        \"http_status\":$status}"
    return 0
  fi
  if [ "$status" -ge 400 ]; then
    log "WARN: Spotify returned HTTP $status for track 
        $track_id"
    echo "{\"error\":\"SPOTIFY_HTTP_${status}\",
        \"http_status\":$status}"
    return 0
  fi
...
# CSV format: path,db_duration,tidal_id,beatport_id,
    qobuz_id,spotify_id
tail -n +2 "$INPUT" | while IFS=, read -r path db_duration 
    tidal_id beatport_id qobuz_id spotify_id; do
  ROW_INDEX=$((ROW_INDEX + 1))
  ...
  log "Processing row $ROW_INDEX: path='${path}', 
    db_duration='${db_duration}', tidal='${tidal_id}', 
    beatport='${beatport_id}', qobuz='${qobuz_id}', 
    spotify='${spotify_id}'"
  ...
  log "Row $ROW_INDEX processed. Sleeping $
    {RATE_LIMIT_DELAY}s..."
...
```
brew install jq           # or your distro's package 
    manager
```
...
Reads NDJSON from metadata_output_full.ndjson (generated 
    by harvest_metadata_full.sh).
Each line: {
  "path": "...",
  "db_duration": <seconds or null>,
  "tidal": {...}, "beatport": {...}, "qobuz": {...}, 
    "spotify": {...},
  "row_index": N
}
...
INPUT_NDJSON = Path(os.environ.get("OUTPUT_NDJSON", 
    "metadata_output_full.ndjson"))
OUTPUT_CSV   = Path("metadata_canonical.csv")

def extract_tidal_track_info(tidal_json: Any) -> Tuple
...
def extract_beatport_track_info(bp_json: Any) -> Tuple
    [Optional[str], Optional[float], Optional[str], 
    Optional[str], Optional[float]]:
    """
    ...
    if not isinstance(bp_json, dict):
        return None, None, None, None, None
    bp_id = str(bp_json.get("id")) if "id" in bp_json else 
        None
    ...
    genre = None
    genre_obj = bp_json.get("genre") or bp_json.get
        ("primary_genre")
    if isinstance(genre_obj, dict):
        genre = genre_obj.get("name")
    ...
    duration_s = None
    if "length_ms" in bp_json and bp_json["length_ms"] is 
        not None:
        try:
            duration_s = float(bp_json["length_ms"]) / 
                1000.0
        except (TypeError, ValueError):
            duration_s = None
...
def extract_qobuz_track_info(qb_json: Any) -> Tuple
    [Optional[str], Optional[float], Optional[str], 
    Optional[str], Optional[float]]:
    """
    ...
    if not isinstance(qb_json, dict):
        return None, None, None, None, None
    qb_id = str(qb_json.get("id")) if "id" in qb_json else 
        None
    ...
    genre = None
    genre_obj = qb_json.get("genre") or qb_json.get
        ("genre_info")
    if isinstance(genre_obj, dict):
        genre = genre_obj.get("name")
    elif isinstance(genre_obj, list) and genre_obj:
    ...
    duration_s = None
    if "duration" in qb_json and qb_json["duration"] is 
        not None:
        try:
...
def extract_spotify_track_info(sp_json: Any) -> Tuple
    [Optional[str], Optional[float], Optional[str], 
    Optional[str], Optional[float]]:
    """
    ...
    For BPM/key, you'd typically use /v1/audio-features/
        {id} separately; this simple
    version assumes they might be top-level if you enrich 
        JSON.
    """
    ...
    duration_s = None
    if "duration_ms" in sp_json and sp_json["duration_ms"] 
        is not None:
        try:
            duration_s = float(sp_json["duration_ms"]) / 
                1000.0
        except (TypeError, ValueError):
            duration_s = None
    ...
    key = sp_json.get("key")  # you may later map numeric 
        key -> string key

    genre = None
    genre_obj = sp_json.get("genre") or sp_json.get
        ("genres")
...
def main():
    ...
    with INPUT_NDJSON.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            tidal_id, tidal_bpm, tidal_key, tidal_genre, 
                tidal_duration_s = extract_tidal_track_info
                (tidal_json)
            bp_id, bp_bpm, bp_key, bp_genre, bp_duration_s 
                = extract_beatport_track_info(bp_json)
            qb_id, qb_bpm, qb_key, qb_genre, qb_duration_s 
    ...
    fieldnames = [
        ...
        "path",
        "db_duration",
        "tidal_id", "beatport_id", "qobuz_id", 
            "spotify_id",
        "tidal_bpm", "beatport_bpm", "qobuz_bpm", 
            "spotify_bpm", "canonical_bpm",
        "tidal_key", "beatport_key", "qobuz_key", 
            "spotify_key", "canonical_key",
        "tidal_genre", "beatport_genre", "qobuz_genre", 
            "spotify_genre", "canonical_genre",
        "tidal_duration_s", "beatport_duration_s", 
            "qobuz_duration_s", "spotify_duration_s",
    ]

    with OUTPUT_CSV.open("w", encoding="utf-8", 
        newline="") as f_out:
...
def choose_canonical_duration(row: dict) -> Tuple[float, 
    str]:
    """
    Choose canonical duration (in seconds) and source 
        provider.
    ...
    for key in ["beatport_duration_s", "qobuz_duration_s", 
        ...
        if v not in (None, "", "null"):
            try:
                return float(v), key.split("_")[0].replace
                    ("duration", "")
            except ValueError:
                continue
    return None, None
    ...
    with Path(CANONICAL_CSV).open(newline="", 
        ...
        for row in reader:
            ...
            if not path:
                continue
            db_duration = None
            if row.get("db_duration") not in (None, "", 
                "null"):
                try:
            ...
            canon_dur, source = choose_canonical_duration
                (row)
...
// Assumes PARSED_SERVICE is set to the appropriate value, 
    e.g. "spotify"
...
if (service === "spotify") {
    ...
    if (t) {
        ...
        record.track_number = t.track_number ?? null;
        record.disc_number = t.disc_number ?? null;
        // If you also call /audio-features, you'd merge 
            bpm/key here.
    }
}
...
if (service === "qobuz") {
    ...
    record.artists = [t.performer?.name || t.artist?.name].
        filter(Boolean);
    record.album = t.album?.title || null;
    record.duration_ms = t.duration != null ? t.duration * 
        1000 : null;
    record.isrc = t.isrc || null;
    record.release_date = t.release_date || t.album?.
        release_date || null;
    record.album_art_url = t.image?.large || t.image?.
        medium || t.image?.small || null;
    record.track_number = t.track_number ?? null;
    record.disc_number = t.media_number ?? null;
    record.genre = t.genre?.name || null;
    if (t.digital_booklet_url) {
        record.pdf_companion_urls.push(t.
    }
}

// Similarly implement for tidal, beatport, apple, itunes, 
    musicbrainz, etc.
...
const iterationInput = pm.iterationData.get("input") || pm.
    environment.get("REQUEST_INPUT");
let agg;
try {
    agg = JSON.parse(pm.environment.get
        ("AGGREGATED_METADATA") || "{}");
...
const order = ["qobuz", "tidal", "beatport", "apple", 
    "itunes", "spotify", "musicbrainz"];
...
function makeSource(serviceName) {
    ...
    return {
        ...
        artist_name: (r.artists && r.artists.length ? r.
            artists.join(", ") : null),
...
def main():
    ...
    with NDJSON_PATH.open(encoding="utf-8") as f:
        for line in f:
            ...
            cur.execute("""
              INSERT INTO library_tracks
                    (library_track_key, title, artist, 
                        album, duration_ms,
                     isrc, release_date, explicit, 
                        best_cover_url, lyrics_excerpt,
                     genre, bpm, musical_key)
              VALUES 
                (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
              ON CONFLICT(library_track_key) DO UPDATE SET
            ...
            for src in sources:
                ...
                """, (
                    ...
                    src.get("isrc"),
                    src.get("album_art_url"),
                    json.dumps(src.get("pdf_companions") 
    ...
    conn.commit()
    conn.close()
    print("Imported hoarding NDJSON into library_tracks 
        and library_track_sources.")
...
def cascade(*sources: TrackMetadata, strategies=None) -> 
    TrackMetadata:
    """
    Merge metadata objects with precedence given by order 
        in `sources`.
    strategies: optional dict[field_name -> 'prefer'|
        'replace'|'append'].
    ...
    for src in sources[1:]:
        for field in fields(TrackMetadata):
            ...
            if strategy == "replace":
                ...
                if val in (None, ""):
                    setattr(merged, name, other)
                elif isinstance(val, str) and isinstance
                    (other, str):
...
def get_track_metadata_from_db(track_key: str, db_path: 
    str) -> TrackMetadata:
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
    cur.execute("""
      SELECT service, metadata_json, genre, bpm, 
        musical_key, album_title,
             artist_name, duration_ms, isrc, 
                album_art_url, lyrics_excerpt
    ...
    for (service, metadata_json, genre, bpm, musical_key, 
        album, artist,
         duration_ms, isrc, art_url, lyrics_excerpt) in 
            rows:
        md = TrackMetadata(
            title="",     # you can parse from 
                metadata_json or library_tracks separately
            artist=artist or "",
            album=album or "",
            year=None,    # parse from release_date as 
                needed
            isrc=isrc,
            art_url=art_url,
    ...
    # Use precedence order here: e.g., Beatport > Qobuz > 
        Tidal > Spotify
    precedence = ["beatport", "qobuz", "tidal", "apple", 
        "itunes", "spotify", "musicbrainz", "discogs"]
    ordered = []
    for p in precedence:
        for s in sources:
            if s.source == p:
...
DB_PATH_DEFAULT = "/Users/georgeskhawam/Projects/dedupe/
    EPOCH_2026-01-24/music.db"
...
@cli.command()
@click.option("--db", "db_path", default=DB_PATH_DEFAULT, 
    help="Path to music.db")
def recovery(db_path):
    ...
    subprocess.run(["python3", "export_files_for_recovery.
        py"], check=True)

    print("Step 2: Running harvester...")
    subprocess.run(["bash", "harvest_metadata_full.sh"], 
        check=True)

    print("Step 3: Aggregating metadata...")
    subprocess.run(["python3", "aggregate_metadata_full.
        py"], check=True)

    print("Step 4: Evaluating health...")
    subprocess.run(["python3", 
        "evaluate_health_from_canonical.py"], check=True)
...
@click.option("--db", "db_path", default=DB_PATH_DEFAULT, 
    help="Path to music.db")
@click.argument("ndjson_path", type=click.Path
    (exists=True))
def hoard(db_path, ndjson_path):
    """
    Import hoarded metadata (hoarding_export.ndjson) into 
        library_tracks and library_track_sources.
    """
    os.environ["MUSIC_DB"] = db_path
    os.environ["HOARDING_NDJSON"] = ndjson_path
    # You can adjust import_hoarding_ndjson.py to use this 
        env var; or call it with args.
    subprocess.run(["python3", "import_hoarding_ndjson.
        py"], check=True)
...
# Recovery mode
python3 cli.py recovery --db "/Users/georgeskhawam/
    Projects/dedupe/EPOCH_2026-01-24/music.db"

# Hoarding mode (after you've run your Postman hoarding 
    exporter and saved to hoarding_export.ndjson)
python3 cli.py hoard hoarding_export.ndjson --db "/Users/
    georgeskhawam/Projects/dedupe/EPOCH_2026-01-24/music.
    db"
```
...
Central place to retrieve and refresh API tokens for the 
    various providers.

Usage (from shell):
  python3 auth_manager.py spotify-access-token

Will print a valid Spotify access token to stdout and 
    refresh if needed.

You MUST:
  - Set SPOTIFY_CLIENT_ID and SPOTIFY_CLIENT_SECRET in 
    env_exports.sh (or shell env).
  - Optionally fill in manual tokens for other providers 
    and plug in refresh logic later.
"""
...
def refresh_spotify_access_token() -> str:
    """
    Uses client credentials flow to get an app-only 
    ...
    if not client_id or not client_secret:
        raise RuntimeError("SPOTIFY_CLIENT_ID or 
            SPOTIFY_CLIENT_SECRET missing in environment")
    ...
    resp = requests.post("https://accounts.spotify.com/api/
        token", headers=headers, data=data, timeout=15)
    if resp.status_code != 200:
        raise RuntimeError(f"Spotify token request failed: 
    ...
    if not access_token:
        raise RuntimeError(f"Spotify token response 
            missing access_token: {body}")

    tokens = load_tokens()
    tokens.setdefault("spotify", {})
    tokens["spotify"]["access_token"] = access_token
    # Subtract a small safety margin so we refresh 
        slightly early
    tokens["spotify"]["expires_at_unix"] = now_unix() + int
        (expires_in) - 60
    save_tokens(tokens)
...
def get_tidal_access_token() -> str:
    """
    Placeholder:
    - If you have a stable refresh token and an auth 
        endpoint, you can implement refresh here.
    - Otherwise, you can store a manually obtained 
        access_token into tokens.json and just reuse it.
    """
    tokens = load_tokens()
    td = tokens.get("tidal") or {}
    access_token = td.get("access_token")
    # You could check td["expires_at_unix"] here and 
        refresh if you know the refresh flow.
    # For now, just return whatever is there.
    if not access_token:
        raise RuntimeError("No TIDAL access_token in 
            tokens.json; please populate manually or 
...
def get_qobuz_user_auth_token() -> str:
    ...
    - You might implement login with username/password, or 
        reuse a recorded user_auth_token.
    """
    tokens = load_tokens()
    qb = tokens.get("qobuz") or {}
    token = qb.get("user_auth_token")
    if not token:
        raise RuntimeError("No QOBUZ user_auth_token in 
            tokens.json; please populate manually or 
...
def get_beatport_access_token() -> str:
    ...
    - Typically requires scraping login OR reusing a 
    ...
    if not token:
        raise RuntimeError("No Beatport access_token in 
            tokens.json; please populate manually or 
            implement login flow")
    return token
...
def get_apple_music_developer_token() -> str:
    """
    Apple Music dev token is a signed JWT you generate 
    ...
    if not token:
        raise RuntimeError("No Apple Music developer_token 
            in tokens.json")
    return token
...
def main():
    if len(sys.argv) < 2:
        print("Usage: auth_manager.py <command>", file=sys.
            stderr)
        print("Commands:", file=sys.stderr)
        print("  spotify-access-token  # prints a valid 
            Spotify access token", file=sys.stderr)
        print("  tidal-access-token    # prints Tidal 
            access token (if configured)", file=sys.stderr)
        print("  qobuz-user-token      # prints Qobuz 
            user_auth_token (if configured)", file=sys.
            stderr)
        print("  beatport-access-token # prints Beatport 
            access token (if configured)", file=sys.stderr)
        print("  apple-dev-token       # prints Apple 
            Music dev token (if configured)", file=sys.
            stderr)
        sys.exit(1)
...
```
local sp_token
sp_token="$(python3 auth_manager.py spotify-access-token 
    2>/dev/null || true)"
if [ -z "$sp_token" ]; then
  log "WARN: Could not obtain Spotify access token from 
    auth_manager; skipping Spotify for $track_id"
  echo "{}"
  return 0
fi
```
...
```
local td_token
td_token="$(python3 auth_manager.py tidal-access-token 2>/
    dev/null || true)"
if [ -z "$td_token" ] || [ -z "${TIDAL_COUNTRY_CODE:-}" ]; 
    then
  log "WARN: Could not obtain TIDAL access token or 
    country code missing; skipping TIDAL for $track_id"
  echo "{}"
  return 0
fi
```
...
```
local qb_token
qb_token="$(python3 auth_manager.py qobuz-user-token 2>/
    dev/null || true)"
if [ -z "$qb_token" ] || [ -z "${QOBUZ_APP_ID:-}" ]; then
  log "WARN: Qobuz token or QOBUZ_APP_ID missing; skipping 
    Qobuz for $track_id"
  echo "{}"
  return 0
fi
...
local url="${base}?track_id=${track_id}&app_id=$
    {QOBUZ_APP_ID}&user_auth_token=${qb_token}"
```
...
```
local bp_token
bp_token="$(python3 auth_manager.py beatport-access-token 
    2>/dev/null || true)"
if [ -z "$bp_token" ]; then
  log "WARN: No Beatport token from auth_manager; skipping 
    Beatport for $track_id"
  echo "{}"
  return 0
fi
...
export 
    SPOTIFY_CLIENT_SECRET="91cceb8042624f2ea5c4420adbc28334
    "
# Tokens are managed by auth_manager.py; no need for 
    SPOTIFY_ACCESS_TOKEN here.

########## TIDAL – STATIC CONFIG ##########
export TIDAL_COUNTRY_CODE="US"
# Access token is retrieved by auth_manager.py; keep any 
    static config here only.
...
1. Put your real Spotify ID/secret into `env_exports.sh`:
   ...
   ```
   export 
       SPOTIFY_CLIENT_ID="9d833039ed2743948b4fbbca824bc81
       5"
   export 
       SPOTIFY_CLIENT_SECRET="91cceb8042624f2ea5c4420adbc
       28334"
   ```
...
4. Run:
   ...
   source ./env_exports.sh
   python3 auth_manager.py spotify-access-token     # 
       just to test; should print a token
   ./harvest_metadata_full.sh
   ```
...
RATE_LIMIT_DELAY="${RATE_LIMIT_DELAY:-3}"
...
log "Row $ROW_INDEX processed. Sleeping ${RATE_LIMIT_DELAY}
    s..."
sleep "$RATE_LIMIT_DELAY"
```
...
# Max requests per second (float). We'll use 1 / MAX_RPS 
    as minimum interval.
...
# Global per-row delay as a "soft" throttle across all 
    providers
export RATE_LIMIT_DELAY="1"    # seconds between rows (was 
    3)
```
...
# If MAX_RPS is 0 or unset, we default to a safe low 
    frequency (1 req/5s).
...
SPOTIFY_MIN_INTERVAL="$(min_interval_for_rps "$
    {SPOTIFY_MAX_RPS:-0.5}")"
TIDAL_MIN_INTERVAL="$(min_interval_for_rps "$
    {TIDAL_MAX_RPS:-0.25}")"
QOBUZ_MIN_INTERVAL="$(min_interval_for_rps "$
    {QOBUZ_MAX_RPS:-0.25}")"
BEATPORT_MIN_INTERVAL="$(min_interval_for_rps "$
    {BEATPORT_MAX_RPS:-0.25}")"

rate_limit_sleep() {
  # $1 = provider name (spotify|tidal|qobuz|beatport)
  local provider="$1"
  local now
  now=$(date +%s.%3N)  # seconds with milliseconds; 
    requires GNU date
  ...
  local elapsed
  elapsed=$(awk -v n="$now" -v l="$last_call" 'BEGIN { 
    printf "%.3f\n", n - l }')

  # how much to sleep
  local sleep_needed
  sleep_needed=$(awk -v mi="$min_interval" -v e="$elapsed" 
    'BEGIN { d = mi - e; if (d < 0) d=0; printf "%.3f\n", 
    d }')

  if awk 'BEGIN { exit !('"$sleep_needed"' > 0) }'; then
    log "Rate-limit: sleeping ${sleep_needed}s before 
        calling ${provider}"
    sleep "$sleep_needed"
  fi
...
get_spotify_full() {
  ...
  # We'll handle status & 429 below in a slightly updated 
    way
  ...
}
```
...
```
/Users/georgeskhawam/Projects/dedupe/metadata_guide.md
```
...
@dataclass
class ProviderTrack:
    service: str                  # 'spotify','qobuz',
        'tidal','beatport','apple'
    ...
    key: Optional[str] = None
    genre: Optional[str] = None
    match_confidence: str = "none"  # 'exact','strong',
        'medium','weak','none'
...
```
def resolve_file_metadata(file_row: dict, mode: str) -> 
    Dict:
    """
    mode: 'recovery' or 'hoarding'
    file_row: {
    ...
    Returns a result dict summarizing decisions (for 
        logging/testing).
    """
    state = "INIT"
    ...
    log_state("Checking for existing provider IDs and 
        ISRC")
    ...
    if any(provider_ids.values()):
        ...
        for svc, track_id in provider_ids.items():
            if not track_id:
                continue
            pt = fetch_by_id(svc, track_id)  # you 
                implement this
            if pt:
                pt.match_confidence = "exact"
                matches.append(pt)
        if matches:
            log_state(f"Got {len(matches)} matches from 
                provider IDs")
        else:
            log_state("No matches returned for existing 
                provider IDs")

    # If no matches yet and we have ISRC, try 
        RESOLVE_BY_ISRC
    if not matches and tag_isrc:
        state = "RESOLVE_BY_ISRC"
        log_state(f"Trying ISRC-based resolution with 
            {tag_isrc}")
        matches.extend(resolve_by_isrc(tag_isrc))  # you 
            implement

    # If still no usable matches, try strong artist+title 
        search
    if not matches:
        ...
        if artist and title:
            ...
            log_state(f"Trying strong query with artist='
                {artist}', title='{title}'")
            strong_matches = resolve_by_artist_title
                (artist, title, measured_duration, mode)
            matches.extend(strong_matches)

    # If still nothing or weak, try duration-guided search 
        by title/fname
    if not matches or all(m.match_confidence in ("medium", 
        "weak", "none") for m in matches):
        state = "RESOLVE_BY_DURATION_GUIDED_QUERY"
        query_title = file_row.get("derived_title") or 
            file_row.get("tag_title")
        if query_title:
            log_state(f"Trying duration-guided search with 
                query_title='{query_title}'")
            dg_matches = resolve_by_duration_guided_query
                (query_title, measured_duration, mode)
            matches.extend(dg_matches)

    # Optional: If you have AcoustID/MusicBrainz, insert 
        RESOLVE_BY_ACOUSTID here

    state = "EVALUATE_MATCHES"
    log_state(f"Evaluating {len(matches)} matches")

    # Filter & categorize matches
    exact_matches = [m for m in matches if m.
        match_confidence == "exact"]
    strong_matches = [m for m in matches if m.
        match_confidence == "strong"]
    medium_matches = [m for m in matches if m.
        match_confidence == "medium"]
    weak_matches = [m for m in matches if m.
        match_confidence == "weak"]

    # Decide if we have usable duration for recovery
    usable_for_recovery = []
    if exact_matches or strong_matches:
        usable_for_recovery = exact_matches + 
            strong_matches
    elif mode == "recovery":
        # Allow medium matches for recovery duration if 
            needed
        usable_for_recovery = medium_matches or 
            weak_matches
    ...
    if usable_for_recovery:
        ...
        log_state(f"Choosing canonical duration from {len
            (usable_for_recovery)} candidates")
        canonical_duration, canonical_source = 
            choose_canonical_duration_from_matches(
            usable_for_recovery
        )
        metadata_health, metadata_health_reason = 
            classify_health(
        ...
        log_state("No usable candidates for recovery 
            duration")

    # For hoarding: only exact/strong
    canonical_summary = None
    sources_for_db = []
    if mode == "hoarding" and (exact_matches or 
        strong_matches):
        state = "CHOOSE_CANONICAL_FOR_HOARDING"
        high_conf = exact_matches + strong_matches
        canonical_summary, sources_for_db = 
            build_canonical_for_hoarding(high_conf)
...
```
def fetch_by_id(service: str, track_id: str) -> Optional
    [ProviderTrack]:
    # Call your existing provider-specific functions from 
        harvest layer or Postman exports.
...
def resolve_by_artist_title(artist: str, title: str, 
    measured_duration: Optional[float], mode: str) -> List
    [ProviderTrack]:
    # For each service:
    #   - Build query "artist title"
    #   - Score candidates by title/artist similarity + 
        duration_delta
    #   - If similarity high and delta small -> 'strong'
    #   - Else if only moderate -> 'medium'
    raise NotImplementedError

def resolve_by_duration_guided_query(query_title: str, 
    measured_duration: Optional[float], mode: str) -> List
    [ProviderTrack]:
    # Use title-only (or fuzzy) queries.
    # Keep results whose durations are close to 
        measured_duration.
    # Label 'medium' or 'weak'.
    raise NotImplementedError

def choose_canonical_duration_from_matches(matches: List
    [ProviderTrack]) -> (Optional[float], Optional[str]):
    # Precedence: beatport > qobuz > tidal > spotify > 
        apple
    order = ['beatport', 'qobuz', 'tidal', 'spotify', 
        'apple']
    for svc in order:
        for m in matches:
            if m.service == svc and m.duration_s is not 
                None:
...
def classify_health(db_dur: Optional[float], canon_dur: 
    Optional[float], tol: float = 2.0):
    if db_dur is None or canon_dur is None:
        return "unknown", "missing_duration"
    delta = db_dur - canon_dur
    if abs(delta) <= tol:
...
def build_canonical_for_hoarding(matches: List
    [ProviderTrack]):
    # Use your cascade logic here to build summary + 
        per-service sources list.
...
no add a very comprehensive fully detaqiled part with the info you mentionned (/Users/georgeskhawam/Projects/dedupe/metadata_add.md) to metadata_guide.md
...
The new material from `/Users/georgeskhawam/Projects/dedupe/metadata_add.md` is now fully integrated into `metadata_guide.md` as Section 10, without changing any of your existing sections.
...
- Read both:
  ...
  - `/Users/georgeskhawam/Projects/dedupe/metadata_guide.md`
  - `/Users/georgeskhawam/Projects/dedupe/metadata_add.md`
...
- A mapping from **Beatsource track IDs → Beatport track IDs**:
  ...
  - Either as a new table:
    ...
    CREATE TABLE IF NOT EXISTS 
        beatsource_beatport_mappings (
      bs_track_id   INTEGER PRIMARY KEY,
      bp_track_id   INTEGER,
      mapping_status TEXT,        -- 'ok','missing',
        'disabled','unknown'
      created_at    TEXT DEFAULT CURRENT_TIMESTAMP
    );
    ```
...
1. **Request: Introspect token for user:bs_sunset**
   ...
   - **Tests**:
     ...
     - Set an environment variable:
       ...
       const hasBsSunset = scopes.includes
           ("user:bs_sunset");
       pm.environment.set("is_beatsource_migrated", 
           hasBsSunset ? "true" : "false");
       ```
   ...
   - **Tests**:
     ...
     - On 200:
       ...
       pm.test("Mapping found", () => {
           pm.expect(data.bp_track_id, "bp_track_id").
               to.be.a("number");
       });
       ```
...
3. **Request: Beatsource→Beatport bulk mapping**
   ...
   - **Tests**:
     ...
     - Parse JSON array, push into an environment variable for use in your scripts:
       ...
       pm.environment.set("bs_bp_bulk_mappings", JSON.
           stringify(arr));
       ```
...
```
if file_row.get("beatsource_track_id") and not file_row.get
    ("beatport_id"):
    bp_id = lookup_bp_from_bs(file_row
        ["beatsource_track_id"], db_path)
    if bp_id:
...
```
# Beatport API base & auth (stubs if you don’t yet have 
    client credentials set up)
export BEATPORT_API_BASE="https://api.beatport.com"

# Standard Beatport access token (for v4 catalog and auth/
    o/introspect)
# This is the same token your Beatport v4 metadata 
    pipeline uses.
export BEATPORT_ACCESS_TOKEN="${BEATPORT_ACCESS_TOKEN:-}"

# Beatsource→Beatport migrator endpoints do NOT require 
    auth,
...
curl -s "https://api.beatport.com/migrator/v1/track/
    9781880"

# Bulk mapping
curl -s "https://api.beatport.com/migrator/v1/track/bulk?
    id=9781880,9781939,9781945"

# Top 10 open format tracks (requires 
    BEATPORT_ACCESS_TOKEN)
curl -s \
  -H "Authorization: Bearer ${BEATPORT_ACCESS_TOKEN}" \
  "${BEATPORT_API_BASE}/v4/catalog/tracks/top/openformat/
    10/"
```
...
```
curl -X POST "https://api.beatport.com/v4/auth/o/token/" \
  -H "Authorization: Basic $(printf '%s:%s' 
...
1. Direct user to:
   ...
   ```
   https://accounts.beatport.com/authorize?
       response_type=code&
...
4. Your backend exchanges `code` for tokens:
   ...
   ```
   curl -X POST "https://api.beatport.com/v4/auth/o/
       token/" \
...
def beatport_refresh_token(tokens: Dict[str, Any]) -> str:
    ...
    ...
    For now, this stub expects you to manually set 
        access_token in tokens.json.
    """
    raise NotImplementedError("Beatport OAuth2 flow not 
        yet implemented")
```
...
def beatport_refresh_token(tokens: Dict[str, Any]) -> str:
    ...
    if not client_id or not client_secret:
        raise ValueError("Beatport client_id and 
            client_secret must be set in tokens.json")
    ...
    req = Request(url, data=data, headers={"Content-Type": 
    ...
    try:
        ...
        raise RuntimeError(f"Beatport token refresh 
...
```
BEATPORT_ACCESS_TOKEN=$(python3 auth_manager.py get 
    beatport)
```
...
- It’s the body from something like:
  ...
  GET https://api.beatport.com/v4/catalog/search/?
      q=Charlotte+de+Witte&type=tracks&per_page=3&page=1
  Authorization: Bearer <BEATPORT_ACCESS_TOKEN>
  ```
