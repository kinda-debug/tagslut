...
Group options:
  ...
  --path PATH            Input path(s) for --m3u (repeatable)

Commands:
  register               Register files in inventory
  check                  Check for duplicate files before downloading
  check-duration         Measure durations and update duration status
  audit-duration         Report duration anomalies
  set-duration-ref       Set trusted duration reference from known-good file
```
...
2. If a strong similarity or exact identity match is found:
   - Display diagnosis:
     ```
