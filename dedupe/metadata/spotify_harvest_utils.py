...
Usage from Python:
    from dedupe.metadata.spotify_harvest_utils import (
        spotify_available,
        get_spotify_headers,
    )
...
Usage from shell (CLI entry point):
    ...
    python3 -m dedupe.metadata.spotify_harvest_utils print-header
    
    # Check if Spotify is available (exit code 0 = yes, 1 = no)
    python3 -m dedupe.metadata.spotify_harvest_utils check
    
    # Print full header line for curl -H
    python3 -m dedupe.metadata.spotify_harvest_utils curl-header
...
logger = logging.getLogger("dedupe.metadata.spotify_harvest_utils")
...
def _cli_main() -> int:
    ...
    """
    if len(sys.argv) < 2:
        print("Usage: python3 -m dedupe.metadata.spotify_harvest_utils <command>", file=sys.stderr)
