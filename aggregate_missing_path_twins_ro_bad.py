#!/usr/bin/env python3
# Proves: deterministic aggregation of existing checksum-match CSVs, incorporating read-only /Volumes/bad existence data.
# Does not prove: filesystem correctness, checksum correctness, or provenance beyond the input CSV contents.
...
OUT_MATCHES = BASE / "missing_paths_RECOVERY_TARGET_checksum_matches_ro_bad.csv"
OUT_BY_PREFIX = BASE / "missing_paths_checksum_twins_by_prefix_ro_bad.csv"
OUT_COLLAPSE = BASE / "missing_paths_checksum_twin_collapse_ro_bad.csv"
OUT_ORPHANS = BASE / "missing_paths_without_any_checksum_twin_ro_bad.csv"
...
# Build /Volumes/bad existence map
bad_exists = {}
for path in BAD_EXISTENCE_FILES:
    with path.open(newline="") as f:
        ...
        for row in reader:
            ...
            if p is None:
                continue
            bad_exists[p] = 1 if row.get("exists") == "1" else 0
...
with INPUT_MATCHES.open(newline="") as f_in, OUT_MATCHES.open("w", newline="") as f_out:
    ...
    for row in reader:
        ...
        match_exists = ""
        if match_path and match_path.startswith(BAD_PREFIX + "/"):
            exists_val = bad_exists.get(match_path)
            match_exists = "" if exists_val is None else str(exists_val)
        elif match_prefix.startswith(BAD_PREFIX + "/"):
            # Fall back to prefix-only match if path is missing.
            match_exists = ""
...
# Step 1 dedup per (relative_path, checksum, match_prefix), using match_exists for /Volumes/bad
# Matches in /Volumes/bad are only counted when match_exists == 1.
dedup_triplets = set()
prefix_to_paths = {}
prefix_to_checksums = {}
prefix_to_rows = {}
...
with OUT_MATCHES.open(newline="") as f:
    ...
    for row in reader:
        ...
        is_bad = (match_prefix or "").startswith(BAD_PREFIX + "/")
        valid_match = False
...
# Step 3: Orphan detection (after filtering /Volumes/bad matches by existence)
...
print("/Volumes/bad matches counted only when match_exists == 1")
