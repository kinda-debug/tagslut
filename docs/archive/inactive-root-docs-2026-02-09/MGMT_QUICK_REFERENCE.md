...
tagslut mgmt register /path/to/downloads --source bpdl

# Execute (actually save to database)
tagslut mgmt register /path/to/downloads --source bpdl --execute

# Verbose (show each file processed)
tagslut mgmt register /path/to/downloads --source bpdl --execute -v

# Use custom database
tagslut mgmt register /path/to/downloads --source bpdl --db /path/to/music.db --execute
```

**Options**:
- `PATH` - Directory containing FLAC files (required)
- `--source` - Download source: bpdl, tidal, qobuz, legacy (required)
- `--db` - Database path (auto-detected from $TAGSLUT_DB if not provided)
...
```bash
# Check a directory
tagslut mgmt check /path/to/downloads --source bpdl

# Check with stdin
find ~/incoming -name "*.flac" | tagslut mgmt check --source tidal

# Strict mode (reject if same file exists from ANY source)
tagslut mgmt check /path/to/downloads --strict

# Verbose (show which files are safe/conflicts)
tagslut mgmt check /path/to/downloads --source bpdl -v

# Use custom database
tagslut mgmt check /path/to/downloads --source bpdl --db /path/to/music.db
```
...
tagslut mgmt register /old/music --source legacy --execute

# Check if any are already in newer downloads
tagslut mgmt check /old/music --db music.db

# Duplicates can be skipped or moved to "duplicate" folder
# (Future: tagslut recovery --move will handle this)
```
...
tagslut mgmt check ~/Downloads/tiddl --source tidal  # Will be unique!
...
### "Database not found"
Make sure your database exists or set `$TAGSLUT_DB` environment variable:
```bash
...
tagslut mgmt register ~/Downloads/bpdl --source bpdl -v
```
...
### With `tagslut scan` (file scanning)
```bash
# First register downloads
tagslut mgmt register ~/Downloads/bpdl --source bpdl --execute

# Then scan the library
tagslut scan /Volumes/DJSSD/EM/Archive
```

### With `tagslut recovery` (future: moving files)
```bash
# Register
tagslut mgmt register ~/Downloads/bpdl --source bpdl --execute

# Later: move to canonical location
tagslut recovery --move ~/Downloads/bpdl --db music.db
```
...
tagslut mgmt register ~/Downloads/bpdl --source bpdl --db ~/dj-set-a.db --execute

# Download for DJ Set B
tools/get https://www.beatport.com/release/set-b/456
tagslut mgmt register ~/Downloads/bpdl --source bpdl --db ~/dj-set-b.db --execute
```
...
for dir in ~/Downloads/*/; do
  ...
  tagslut mgmt register "$dir" --source "$source" --execute
done
```
...
A: Register anyway. If corruption is detected by `flac -t`, the file will be marked accordingly. The `tagslut recovery` command will handle repair.
...
- ✅ Phase 1: **tagslut mgmt** (you are here)
- ⏳ Phase 2: **tagslut recovery** (move files with safety checks)
