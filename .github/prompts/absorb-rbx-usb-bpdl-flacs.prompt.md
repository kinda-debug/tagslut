...
# Absorb RBX_USB staging/bpdl FLACs back to main staging

## Context

Beatport FLAC downloads were moved to `/Volumes/RBX_USB 1/staging/bpdl/` for
space reasons. They need to be moved back to `/Volumes/MUSIC/staging/bpdl/` so
the normal intake pipeline can process them.

## Source

`/Volumes/RBX_USB 1/staging/bpdl/` — all `.flac` files, flat or nested.

## Destination

`/Volumes/MUSIC/staging/bpdl/` — create if not exists.
...
- Collision handling: if a file with the same name already exists at the
  ...
  `/Volumes/MUSIC/staging/bpdl/_conflicts/` instead and log it.
- Preserve directory structure relative to `/Volumes/RBX_USB 1/staging/bpdl/`.
...
- Log moves to `/Volumes/MUSIC/staging/bpdl/_absorb_log_YYYYMMDD.txt`.
...
```
feat(tools): add script to absorb RBX_USB bpdl FLACs back to staging
```
