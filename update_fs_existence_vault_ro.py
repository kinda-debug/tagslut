#!/usr/bin/env python3
# Proves: read-only existence capture for /Volumes/Vault prefixes and comparison vs prior fs_existence CSVs.
...
comparison_path = base / 'fs_existence_vault_comparison.csv'
...
# Build summary using ro_bad + ro_commune + ro_vault + existing others
summary_path = base / 'fs_existence_summary_by_prefix_ro_bad_commune_vault.csv'
...
for path in base.glob('fs_existence_*.csv'):
    ...
    if prefix.startswith('/Volumes/bad') or prefix.startswith('/Volumes/COMMUNE') or prefix.startswith('/Volumes/Vault'):
        continue
