#!/usr/bin/env python3
# Proves: partitions missing paths into three buckets after existence filtering for /Volumes/bad, /Volumes/COMMUNE, /Volumes/Vault.
...
INPUT_MATCHES = BASE / "missing_paths_RECOVERY_TARGET_checksum_matches_ro_bad_commune_vault.csv"
INPUT_ORPHANS = BASE / "missing_paths_without_any_checksum_twin_ro_bad_commune_vault.csv"

OUT_PRESENT_CONFIRMED = BASE / "missing_paths_bucket_present_bad_commune_vault.csv"
OUT_OTHER_TWINS = BASE / "missing_paths_bucket_other_twins_ro_bad_commune_vault.csv"
OUT_ORPHANS = BASE / "missing_paths_bucket_orphans_ro_bad_commune_vault.csv"
...
# Bucket 1: confirmed twins on /Volumes/bad or /Volumes/COMMUNE or /Volumes/Vault
bucket_confirmed = sorted(p for p in all_paths if path_confirmed_present.get(p))

# Bucket 2: twins exist, but none confirmed on /Volumes/bad or /Volumes/COMMUNE or /Volumes/Vault
bucket_other = sorted(
