...
def register_execute_group(cli: click.Group) -> None:
    ...
    def execute_move_plan(plan_path: Path, db: Path | None, dry_run: bool, verify: bool) -> None:
        ...
        move_rows = _load_move_plan_rows(plan_path.expanduser().resolve())
        if not move_rows:
            click.echo("No MOVE rows found in plan.")
            return
        ...
        if db is not None:
            ...
            try:
                plan_id = ensure_move_plan(
                    conn,
                    plan_key=f"execute.move-plan:{plan_path.resolve()}",
        ...
        click.echo(f"Rows: {len(receipts)}")
        click.echo(f"Moved: {counts['moved']}")
        click.echo(f"Dry-run: {counts['dry_run']}")
        click.echo(f"Skipped (missing): {counts['skip_missing']}")
        click.echo(f"Skipped (dest exists): {counts['skip_dest_exists']}")
