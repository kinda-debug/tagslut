...
# Handover — dedupe Repo

Date: 2026-02-01

## Summary (This Session)

Designed and documented the **Management & Recovery Mode** system for the dedupe CLI:

- **`dedupe mgmt`** — Management mode for inventory tracking, duplicate checking, and M3U generation
- **`dedupe recovery`** — Recovery mode for file operations with move-only semantics
...
### CLI Structure
```
dedupe mgmt [options]       # -m shorthand
dedupe recovery [options]   # -r shorthand
```

### `dedupe mgmt` Features
...
### M3U Generation (dedupe mgmt Responsibility)

**Important:** M3U generation is handled by `dedupe mgmt`, NOT by BeatportDL or other downloaders. BeatportDL does not have a `--m3u` flag.

- `dedupe mgmt --m3u` generates one playlist per item
...
2. Register to inventory: `dedupe mgmt --source X --m3u --check`
3. Review M3U in Roon, verify tracks
4. Move verified tracks: `dedupe recovery --move --zone accepted`
5. Repeat — inventory prevents re-downloads

## Previous Session Summary (2026-01-31)

- Implemented zone subsystem (`dedupe/utils/zones.py`)
- Centralized keeper selection (`dedupe/core/keeper_selection.py`)
...
1. **Implement CLI stubs** for `dedupe mgmt` and `dedupe recovery` in `dedupe/cli/main.py`
...
- Should `dedupe mgmt` wrap the actual downloader, or just process post-download?
