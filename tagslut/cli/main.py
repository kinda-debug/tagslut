...
@metadata.command()
@click.option('--tokens-path', type=click.Path(), help='Path to tokens.json')
@click.option('--no-refresh', is_flag=True, help='Do not attempt token refresh')
def auth_status(tokens_path, no_refresh):
    ...
    for provider, info in status.items():
        ...
        has_token = info.get('has_token')
        expired = info.get('expired')
        if provider in ("itunes", "apple_music"):
            state = "ready"
        elif configured and has_token:
            state = "expired" if expired else "valid"
        elif configured and not has_token:
            state = "not authenticated"
        else:
            state = "not configured"
        click.echo(f"{provider:12s} | {state}")

    click.echo("")
    click.echo("To configure providers:")
    if 'tidal' in status and not status['tidal'].get('configured'):
        click.echo("  tidal:    Run 'tagslut auth login tidal'")
    if 'qobuz' in status and not status['qobuz'].get('configured'):
...
def auth_refresh(provider, tokens_path):
    ...
    if provider == 'beatport':
        ...
        if token:
            ...
            click.echo("Failed. Run 'tagslut auth login tidal' to re-authenticate.")
...
def auth_login(provider):
    """
    Interactive login for providers that require user auth.
    """
    from tagslut.metadata.auth import TokenManager
