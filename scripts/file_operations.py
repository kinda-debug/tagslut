...
def cmd_move_trash(args: argparse.Namespace) -> int:
    """Move duplicate audio files marked 'plan' in dedup report to trash."""
    report_path = Path(
        args.report or "/Volumes/dotad/MUSIC/_DEDUP_REPORT_20251028_014226.csv"
    )
    ...
    try:
        ...
        print(
            f"📊 Summary: {moved_count} moved, {failed_count} failed, {skipped_count} skipped"
        )
...
def cmd_collect_logs(args: argparse.Namespace) -> int:
    ...
    out_file = Path(
        args.out_file
        or f"repair_logs_{datetime.now().strftime('%Y%m%d_%H%M%S')}.tar.gz"
    )
...
def main() -> int:
    ...
    archive_parser.add_argument(
        "--dest", default="archive/", help="Archive destination directory"
    )
    ...
    move_parser.add_argument(
        "--dry-run", action="store_true", help="Dry run mode (no actual moves)"
    )
    move_parser.set_defaults(func=cmd_move_trash)

    # Collect logs command
    logs_parser = subparsers.add_parser(
        "collect-logs", help="Collect repair stderr logs"
    )
